...
def weighting_func (src_dir, **kwargs):
    ...
    warnings.filterwarnings("ignore")
        ...
        # Reads the timestamps.txt file created in the overlap correction step
    Timestamps = pd.read_csv(src_dir + '/timestamps.txt')
    Timestamps = Timestamps.sort_values(by=['Folder'], ascending=True)
    ...
    for i in range (len(idx_ob)-1):
    
            # creates a DataFrame (to process) that takse the rows in between two OBs
        to_proc = Timestamps[Timestamps['Folder'].between(OB_df['Folder'][idx_ob[i]], OB_df['Folder'][idx_ob[i+1]], inclusive=False)]
        
            # loop to create the weightings for each case in between 2 OBs
        for n, val in enumerate(to_proc['Folder'].index):
            Weights.loc[val,'w1'] = (OB_df['Modification (s)'][idx_ob[i+1]]-to_proc['Modification (s)'][val])/(OB_df['Modification (s)'][idx_ob[i+1]] - OB_df['Modification (s)'][idx_ob[i]])
            Weights.loc[val,'w2'] = (to_proc['Modification (s)'][val]-OB_df['Modification (s)'][idx_ob[i]])/(OB_df['Modification (s)'][idx_ob[i+1]] - OB_df['Modification (s)'][idx_ob[i]])
            Weights.loc[val,'OB1'] = OB_df['Folder'][idx_ob[i]]
            Weights.loc[val,'OB2'] = OB_df['Folder'][idx_ob[i+1]]
            
            # this part takes care if there is no OB at the beginning (before one that requires it)
        # if idx_w[0] == 0 :
        #     Weights.loc[0,'w1'] = 0
        #     Weights.loc[0,'w2'] = 1
        #     Weights.loc[0,'OB1'] = OB_df['Folder'][idx_ob[0]]
        #     Weights.loc[0,'OB2'] = OB_df['Folder'][idx_ob[0]]
            
            # this part takes care if there is no OB at the end (after one that requires it)
        if idx_w[-1] == len(Timestamps['Folder'])-1:
            Weights.loc[idx_w[-1],'w2'] = 0
            Weights.loc[idx_w[-1],'w1'] = 1
            Weights.loc[idx_w[-1],'OB1'] = OB_df['Folder'][idx_ob[-1]]
            Weights.loc[idx_w[-1],'OB2'] = OB_df['Folder'][idx_ob[-1]]
    return Weights
