...
# =============================================================================
def stack_transmission_img (dictionary, ref_mask_img, img_mask_img, ref_key = '', print_TFC = False, save_sbkg = False, **kwargs):
    '''
    ...
    **kwargs : TYPE
    ...
    sbkg_refs_list = []
    sbkg_imgs_list = []
    after_sbkg_refs_list = []
    after_sbkg_imgs_list = []
    sbkg_dictionary = {}
    ...
    for key,value in dictionary.items():
        ...
        if key == ref_key:
            for i, ref in zip (tqdm(range(len(value)), desc = 'Processing'),value):
                ...
                sbkg_refs_list.append(sbkg_ref)
                after_sbkg_refs_list.append(ref - sbkg_ref)
            ...
            for i, img in zip (tqdm(range(len(value)), desc = 'Processing'),value):
                ...
                sbkg_imgs_list.append(sbkg_img)
                after_sbkg_imgs_list.append(img - sbkg_img)

        if key!= ref_key and save_sbkg:
            sbkg_dictionary[key + '_sbkg_refs'] = sbkg_refs_list
            sbkg_dictionary[key + '_sbkg_imgs'] = sbkg_imgs_list
            sbkg_dictionary[key + '_after_sbkg_refs'] = after_sbkg_refs_list
            sbkg_dictionary[key + '_after_sbkg_imgs'] = after_sbkg_imgs_list
    ...
    if save_sbkg:
        dictionary.update(sbkg_dictionary)
...
#                        stack_batch_proc
# =============================================================================
def stack_batch_proc (src_dir, dst_dir, ref_mask_img, img_mask_img, ref, proc_folder = [], ref_folder = '15_ref_bb', avg_seq = [stack_avg], save_results = True, final_HE_LE = (False, [3,23],[60,80]), save_sbkg = False, **kwargs):
    '''
    ...
    #         HE_avg = np.nanmean(values[final_HE_LE[1][0]:final_HE_LE[1][1]], axis = 0)
    #         LE_avg = np.nanmean(values[final_HE_LE[2][0]:final_HE_LE[2][1]], axis = 0)

    #         stack_dict[key] = [HE_avg,LE_avg]
#--------------------------------------------------------------------------------------------------------------------------------------
    # new_dst_dir = dst_dir + '/after_avg'
    # if not os.path.exists(new_dst_dir):
    #     os.makedirs(new_dst_dir)
    # save_dict(stack_dict, new_dst_dir)
#--------------------------------------------------------------------------------------------------------------------------------------

    # for key, values in stack_dict.items():
    #     if '_ob' in key:
    #         for img in values:
    #             mod_for_ob (img, roi_original=[0,0,50,512], roi_mod=[462,0,50,512])
    
#--------------------------------------------------------------------------------------------------------------------------------------
    # new_dst_dir = dst_dir + '/after_ob_corr'
    # if not os.path.exists(new_dst_dir):
    #     os.makedirs(new_dst_dir)
    # save_dict(stack_dict, new_dst_dir)
#--------------------------------------------------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------------------------------------------
    # new_dst_dir = dst_dir + '/after_scrubbing'
    # if not os.path.exists(new_dst_dir):
    #     os.makedirs(new_dst_dir)
    # save_dict(stack_dict, new_dst_dir)
#--------------------------------------------------------------------------------------------------------------------------------------
    img_registration_dict (stack_dict, proc_folder, ref= np.zeros([512,512]), num_iter = 3, method= '1',  show_corr = False, **kwargs)
        # computes the transmission images
    stack_transmission_img (stack_dict, ref_mask_img, img_mask_img, ref_key = ref_folder, save_sbkg = save_sbkg, **kwargs)

    
        # saves the results 
    if save_results and final_HE_LE[0] == True:
        #save_dict_energies(stack_dict, dst_dir, **kwargs)
        save_stack_energies (stack_dict, dst_dir, HE_slice = [final_HE_LE[1][0], final_HE_LE[1][1]], LE_slice = [final_HE_LE[2][0],final_HE_LE[2][1]], **kwargs)
        
    elif save_results and final_HE_LE[0] == False:
...
#                        save_dict_energies
# =============================================================================
def save_dict_energies (dictionary, dst_dir, start_slice = '', testing_mode = False, **kwargs):
    '''
    takes a dictionay and saves each key and its values in a target directory
    possible kwargs:
        start_slice : int
    ...
    for key, values in dictionary.items():
        ...
        for img in values:
            ...
            if testing_mode:
                ...
                # creates the saving path 
        file_name_HE = os.path.join(dst_dir, key, key + '_HE.fits' )
        file_name_LE = os.path.join(dst_dir, key, key + '_LE.fits' )
        
                # utilize pyerre's function to save each image in the folder
        write_img(np.flip(values[0], 0), file_name_HE, base_dir='', overwrite=False)
        write_img(np.flip(values[1], 0), file_name_LE, base_dir='', overwrite=False)
...
# This function takes a region on the left side of the image (vertical rectangle)
# and mirrors it to set it as the right part of the same image 

def mod_for_ob (img, roi_original=[0,0,50,512], roi_mod=[462,0,50,512]):
    
        #for this you can use np.flip(roi_img,1) or np.fliplr(roi_img)
    roi_orig_mod = np.fliplr(crop_img(img, roi_original))    
    img[roi_mod[1]:roi_mod[1]+roi_mod[3],roi_mod[0]:roi_mod[0]+roi_mod[2]] = roi_orig_mod
...
def img_registration (img, ref, num_iter = 3, method = '1', show_corr = False, **kwargs):
    '''
    ...
    if method =='2':
        result = ird.similarity(ref, img, numiter=num_iter)
        
        assert "timg" in result
        
        if show_corr:
            ird.imshow(ref, img, result['timg'])
...
# =============================================================================
#                       img_registration_dict for experiments with no scrubbing and no image timestamps
# =============================================================================

### tTHIS FUNCTION IS FOR NON AVERAGED EXPERIMENTS
# FOR AVERAGED EXPERIEMTNS MODIFY THE SECOND TO LAST AND ;LAST FOR LOOPS

def img_registration_dict (stack_dict, proc_folder, ref= np.zeros([512,512]), num_iter = 3, method= '1',  show_corr = False, **kwargs):
    
    for key, value in stack_dict.items():
        new_list = []
        if key in proc_folder:

            print('Performing Image Registration for exp ' + key)
            for img in value:

                new_list.append(img_registration (img, ref, num_iter = num_iter, method = method, show_corr = False))
