...
def read_n_sort_metadata (metadata_list, max_number_exps = 100):
    ...
    for idx, item in enumerate(metadata_list):
        ...
        global_exp_new_index = data.loc[data['metadata'].str.contains("HIERARCH system/Sample")].index.values[0]
        global_exp_new = re.search("'(.*)'", data['metadata'][global_exp_new_index]).group(1)
        ...
        global_exp_old_index = data.loc[data['metadata'].str.contains("HIERARCH system/Sample")].index.values[0]
            # (loc: busca en data, en la columna de metadata, un string que contenga (string)... entonces dame el indice y conviertelo a lista)
        global_exp_old = re.search("'(.*)'", data['metadata'][global_exp_old_index]).group(1)
...
def copy_sorted_files(src_dir, dst_dir, Sorted_dataframe, start_numbering = 3, action = 'copy'):
    
    import shutil
    import pandas as pd

        # tells the first index. If the files are reference, start is 0, else 3 (ob, ref, ob)
    idx = start_numbering
    ...
    #for glob_exp in column_names:
        ...
        dst_folder = dst_dir + '/' + format(idx, '03d') + '_' + glob_exp
