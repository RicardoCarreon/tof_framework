...
#                         stack_avg
# =============================================================================
def stack_avg(base_folder, **kwargs):
    '''
    ...
    for i in range(nimg):
        ...
        for j in range (zimg):
            ...
            if type(base_folder[j][i]) == np.ndarray or type(base_folder[j][i]) == tuple:
                img = base_folder[j][i]
            ...
            if j == 0:
                img_tot = (img[0].astype(float), img[1])
            else:
                img_tot = ((img_tot[0] + img[0]), img[1])
        
            # Save the image in the same variable, locally.
        base_folder.append(((img_tot[0] / zimg), img_tot[1]))
...
#                         exec_averaging
# =============================================================================
def exec_averaging (stack_dict, seq = '', count_time = False, avg_stack = False, **kwargs):
    '''
    ...
    **kwargs : extra arguments. dict
    ...
    for key,values in stack_dict.items():
        ...
        for j,folder in zip (tqdm(range(len(new_values)), desc = 'Processing'),new_values):
            
            if kwargs.get ('binning') == True:
                row_values = []
                folder = binning_proc(folder, **kwargs)
                for i,img in enumerate(folder):
                    row_values.append( exec_proc(img,seq,**kwargs))
                ...
                for i,img_dir in enumerate(folder):
                    ...
                    if type(img_dir) == str:
                        ...
                        new_values[j][i] = exec_proc(img,seq,**kwargs)
                    else:
                        new_values[j][i] = exec_proc(img_dir[0],seq,**kwargs)
                    
            # performs the averaging
        if avg_stack:
            stack_avg(new_values)
            print('Performed averaging of stack')
            # If there is any function performed after the averaging , this part does it
        # for n,img in enumerate(new_values):
        #     new_values[n] = exec_proc(img,seq,**kwargs)
...
#                          create_SBKG_Wmask
# =============================================================================
def SBKG_Wmask (img, BB_mask_img, **kwargs):
    """
    ...
    BB_mask_img_data = BB_mask_img[0]
...
def scrubbing_corr (src_img, ob_01_img, ob_02_img, weight_01 = 0.5, weight_02 = 0.5, **kwargs):
    ...
    header = src_img[1]
    return ((src_img[0]/(ob_01_img * weight_01 + ob_02_img * weight_02)), header)
...
# =============================================================================
def stack_scrubbing_corr (dictionary, weights_DataFrame, avg_stack=False, **kwargs):
    '''
    ...
    for key, value in dictionary.items():
        ...
        if key in weights_DataFrame.Folder.values:
            ...
            ob_01 =[]
            for folder in dictionary[row_data['OB1'].values[0]]:
                for arr in folder:
                    ob_01.append(arr[0])
            ob_01_img = np.nanmean(ob_01, axis=0)

            ob_02 =[]
            for folder in dictionary[row_data['OB2'].values[0]]:
                for arr in folder:
                    ob_02.append(arr[0])
            ob_02_img = np.nanmean(ob_02, axis=0)

                # with the extracted values, operate to obrain the scrubbing correction
            #save_path = 'D:/External_Tony/Processed/Cuvettes/after_scrubbing'
            if avg_stack:

                folder = []
                new_key = key+'_folder_00'
                new_dict[key][new_key] = {}
                for i, src_img in enumerate(value):
                    folder.append(scrubbing_corr (src_img, ob_01_img, ob_02_img, weight_01,weight_02, **kwargs))
                
                new_dict[key][new_key] = folder
                    
            else:
                
                for n, sub_folder in enumerate(value):
                    folder = []
                    new_key = 'folder_' + str(n).zfill(2)
                    new_dict[key][new_key] = {}
                    for i, src_img in enumerate(sub_folder):
    
                        folder.append(scrubbing_corr (src_img, ob_01_img, ob_02_img, weight_01,weight_02, **kwargs))
                        #folder.append(src_img)
...
def TFC_corr(src_img, ref_img, nca=[0,0,0,0], **kwargs):
    ...
    if sum(nca) == 0:
        ...
        print('area cropped (img) = {}'.format( np.average(crop_img(src_img, nca))))
        print('area cropped (ref) = {}'.format( np.average(crop_img(ref_img, nca))))
...
# =============================================================================
def stack_transmission_img (dictionary, ref_mask_img, img_mask_img, ref_key = '', scrubbing = True, print_TFC = False, **kwargs):
    '''
    ...
    **kwargs : TYPE
    ...
    for key,value in dictionary.items():
        ...
        if key == ref_key:
            ...
            ref_folder_avg =[]
            for folder in dictionary[ref_key].values():
                pulse = []

                for arr in folder:
                    pulse.append(arr[0])
                ref_folder_avg.extend([pulse])

            ref_values = [sum(x) / len(x) for x in zip(*ref_folder_avg)]
            # if scrubbing == True:
            #     print('entered in scrubbing True in transmission img')
                
            #     avg_dict_imgs(value, seq = [stack_avg])#, **kwargs )
                
            #     for folder in value:
    
            #         # save_path_3 = 'D:/External_Tony/Processed/Cuvettes/ref_after_sbkg/' + 'ref_min_sbkg' + folder
            #         # if not os.path.exists(save_path_3):
            #         #     os.makedirs(save_path_3)
            #         for i, ref in zip (tqdm(range(len(value[folder])), desc = 'Processing'),value[folder]):
    
            #             sbkg_ref = SBKG_Wmask (ref, ref_mask_img, **kwargs)
            #             value[folder][i] = (ref[0] - sbkg_ref[0], ref[1])
            # else:
            ref_corr = []
            for i, ref in zip (tqdm(range(len(ref_values)), desc = 'Processing'),ref_values):

                sbkg_ref = SBKG_Wmask (ref, ref_mask_img, **kwargs)
                ref_corr.append(ref - sbkg_ref[0])
            ...
            if scrubbing == True:
                for folder in value:
                    # save_path = 'D:/External_Tony/Processed/Cuvettes/sbkg_img/' + 'SBKG_' + folder
                    # save_path_2 = 'D:/External_Tony/Processed/Cuvettes/after_sbkg/' + 'img_min_sbkg' + folder
    
                    # if not os.path.exists(save_path):
                    #     os.makedirs(save_path)
                    # if not os.path.exists(save_path_2):
                    #     os.makedirs(save_path_2)
                    for i, img in zip (tqdm(range(len(value[folder])), desc = 'Processing'),value[folder]):
                        sbkg_img = SBKG_Wmask (img, img_mask_img, **kwargs)
    
                        value[folder][i] = (img[0] - sbkg_img[0], img[1])
                        # test = img[0] - sbkg_img[0]
                        # fits.writeto(save_path + '/sbkg_img_'+ str(i).zfill(5) +  '.fits', sbkg_img[0])
                        # fits.writeto(save_path_2 + '/after_sbkg_corr_'+ str(i).zfill(5) +  '.fits', test)
            else:
                for i, img in zip (tqdm(range(len(value)), desc = 'Processing'),value):
                    sbkg_img = SBKG_Wmask (img, img_mask_img, **kwargs)
                    value[i] = (img[0] - sbkg_img[0], img[1])
                    
        # identify the reference values in the dictionary
    ref_dict = ref_corr#dictionary[''.join(ref_key)]
    ...
    for key,value in dictionary.items():
        ...
        if scrubbing == False:
            for i, src_img, ref_img in zip (np.arange(0, len(value)),value, ref_dict):
                ...
                if  print_TFC:
                    print(TFC)
                value [i] = ((src_img[0]/ref_img)*TFC, src_img[1])
        else:
        ...
        #     print(key)
            for folder in value:
                print('ref folder processing: {}'.format(folder))
                for i, src_img, ref_img in zip (np.arange(0, len(value[folder])),value[folder], ref_dict):
                    #sbkg_ref = SBKG_Wmask (ref_img, ref_mask_img, **kwargs)
                    #ref_corr_img = ref_img - sbkg_ref
                        # intensity correction 
                    TFC = TFC_corr(src_img[0], ref_img, **kwargs)
                    if  print_TFC:
                        print(TFC)
                    value[folder][i] = ((src_img[0]/ref_img)*TFC, src_img[1])
    return 
...
#                        stack_batch_proc
# =============================================================================
def stack_batch_proc (src_dir, dst_dir, ref_mask_img, img_mask_img, reg_img, proc_folder = [], ref_folder = '15_ref_bb', avg_seq = '', save_results = True, scrubbing = True, HE_LE = ([3,23],[60,80]), **kwargs):
    '''
    ...
    if scrubbing:
        ...
        print('Performing Scrubbing Correction')
        stack_dict = stack_scrubbing_corr (stack_dict, weights, **kwargs)
        
    img_registration_dict (stack_dict, proc_folder, reg_img, **kwargs)
    ...
    stack_transmission_img (stack_dict, ref_mask_img, img_mask_img, ref_key = ref_folder, **kwargs)
        ...
        # saves the results 
    if scrubbing == True and save_results == True:
    
        #save_dict(stack_dict, dst_dir, **kwargs)
        for key, value in stack_dict.items():
            new_dst_dir = dst_dir + '/' + key
            if not os.path.exists(new_dst_dir):
                os.makedirs(new_dst_dir)
            save_stack_energies (value, new_dst_dir, HE_slice = [HE_LE[0][0], HE_LE[0][1]], LE_slice = [HE_LE[1][0],HE_LE[1][1]], **kwargs)
            
    elif scrubbing == False and save_results == True:
        save_stack_energies (stack_dict, dst_dir, HE_slice = [HE_LE[0][0], HE_LE[0][1]], LE_slice = [HE_LE[1][0],HE_LE[1][1]], **kwargs)
...
# =============================================================================
def test_filtering_stack (src_dir,dst_dir, test_folder = '', test_seq = [stack_avg], img_number = '', give_img = False, give_stack = False, **kwargs):
    '''
    ...
    if give_stack:
        exec_averaging_old(stack_dict, test_seq, start_slice = 10, end_slice = 20, **kwargs)
        return stack_dict
    ...
    if stack_avg not in test_seq:
        ...
        # executes the averaging process
    exec_averaging_old(stack_dict, test_seq, start_slice = start_slice, end_slice = end_slice, **kwargs)
    ...
    show_img(prep_img_for_display(img[0][0]), title = 'Filtered and Averaged Image #'+str(start_slice))
...
def save_stack_energies (stack_dict, dst_dir, HE_slice = [5, 19], LE_slice = [36,50],start_slice = '', testing_mode = False, **kwargs):
    ...
    for key, values in stack_dict.items():
        ...
        imgs = [img[0] for img in values]
        
        HE_avg = np.nanmean(imgs[HE_slice[0]:HE_slice[1]], axis = 0)
        LE_avg = np.nanmean(imgs[LE_slice[0]:LE_slice[1]], axis = 0)
        
        header_HE = values[int((HE_slice[0]+HE_slice[1])/2)][1]
        header_LE = values[int((LE_slice[0]+LE_slice[1])/2)][1]
            # takes each value in the subfolder (values)
        ...
        if testing_mode:
            ...
            show_img(prep_img_for_display(LE_avg), title = 'Transmission image LE')
                ...
                # creates the saving path 
            file_name_HE = os.path.join(dst_dir , 'HE_stack', 'Transmission_img_HE_' + key + '.fits')#'_HE_' + format(index, '05d') + '.fits')
            file_name_LE = os.path.join(dst_dir , 'LE_stack', 'Transmission_img_LE_' + key + '.fits')#'_LE_' + format(index, '05d') + '.fits')
        
            img_HE = (np.flip(HE_avg, 0), header_HE)
            img_LE = (np.flip(LE_avg, 0), header_LE)
            
                # utilize pyerre's function to save each image in the folder
            write_img(img_HE, file_name_HE, base_dir='', overwrite=False)
            write_img(img_LE, file_name_LE, base_dir='', overwrite=False)
...
#                         exec_averaging
# =============================================================================
def exec_averaging_old (stack_dict, seq = '', count_time = False, **kwargs):
...
import scipy as sp
import cv2 
from img_utils import oversample_img

def img_align(img, ref, rois_list, max_shift=20, subpix=10, dof=['tx','ty','sx','sy'], debug_data={}, print_rois= False, **kwargs):
    ...
    for i, roi_def in enumerate(rois_list):
        ...
        crop_ref_int16 = ref[roi[1]:roi[1]+roi[3], roi[0]:roi[0]+roi[2]]
        
            # get the sub-region in the image to be aligned, including the margins
            # TODO: avoid erros if these margins get over the border
        crop_shift_int16 = img[roi[1]-ky:roi[1]+roi[3]+ky, roi[0]-kx:roi[0]+roi[2]+kx]
...
def img_registration (img, ref, num_iter = '', method = '', rois_list ='', dof=['tx','ty','sx','sy'], **kwargs):
    '''
    ...
    if method =='1':
        shifted, error, diffphase = phase_cross_correlation(ref_mask, img_mask)
        #print(f"Detected subpixel offset (y, x): {shifted}")

        from scipy.ndimage import shift
        corr_img = shift(img, shift=(shifted[0], shifted[1]), mode='constant')
        
        return corr_img, shifted
    
    if method =='2':
        result = ird.similarity(ref, img, num_iter)
        
        assert "timg" in result
    
        return result['timg'], result
    
    if method == '3':
        M = img_align(img, ref, rois_list, max_shift=20, subpix=10, dof=dof, debug_data={})
        corrected_img = sp.ndimage.affine_transform(img, M, order=1)
...
### tTHIS FUNCTION IS FOR NON AVERAGED EXPERIMENTS
# FOR AVERAGED EXPERIEMTNS MODIFY THE SECOND TO LAST AND ;LAST FOR LOOPS

def img_registration_dict (stack_dict, proc_folder, ref= np.zeros([512,512]), num_iter = '', method= '', show_corr = False, **kwargs):
    new_dict = {}
    for key, value in stack_dict.items():
        ...
        if key in proc_folder:
            ...
            new_dict [key] = {}
            print('Performing Image Registration for exp ' + key + ' with method ' + method)
            for sub_key, folder in stack_dict [key].items():
                new_list = []
                for img in folder:
                    new_img, _ = img_registration (img[0], ref[0], num_iter = num_iter, method = method, **kwargs)
                    new_list.append((new_img, img[1]))
                stack_dict [key][sub_key] = new_list
