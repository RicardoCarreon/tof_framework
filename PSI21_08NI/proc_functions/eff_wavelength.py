# -*- coding: utf-8 -*-
"""
Created on Tue Feb  1 15:25:14 2022
...
def give_curve_center_mass (x_array, y_array, degree_fit = 4, plot_curve = True):
    ...
    if plot_curve:
        ...
        plt.title('Curve Fit and Center of Mass')
        plt.plot(x_array,y_array,'b--')
        #plt.plot(statistics.median(x_array), statistics.median(y_array), 'ko')
        #plt.ylim([0,round(np.max(y_array)+1)])
        plt.xlabel(' Wavelenth '+ r'[$\AA$]')
        plt.plot(xtrend,ytrend,'r-', linewidth=0.5)
        plt.plot(CMx,CMy, 'r*')
        plt.grid(True)
...
#                           trapezoid_weight
# =============================================================================
def trapezoid_weight (x_values, y_values, point, base_bot_percentage = 32, base_top_percentage = 8):
    '''
    ...
    idx, val = find_nearest_val (x_values, point)
    
    left_min = idx- int(np.floor(0.005*base_bot_percentage*len(x_values)))
    left_max = left_min + int(np.floor(0.005*(base_bot_percentage-base_top_percentage)*len(x_values)))
   
    mid_min = left_max
    mid_max = idx + int(np.floor(0.005*base_top_percentage*len(x_values)))
    
    right_min = mid_max
    right_max = idx + int(np.floor(0.005*base_bot_percentage*len(x_values)))
    ...
    weights = [*w1,*w2,*w3]
    curve_slice = [*y_values[left_min:left_max], *y_values[mid_min:mid_max], *y_values[right_min:right_max]]
    
    x_val_slice = [*x_values[left_min:left_max], *x_values[mid_min:mid_max], *x_values[right_min:right_max]]
    weighted_array = np.array(weights)*np.array(curve_slice)

    return x_val_slice, weighted_array
...
def convert_tof (tof_values, flight_path = 1, result = 'wavelength', **kwargs):
    conv_res = []
    if result == '':
...
def eff_wvl_beamline( beamline = 'BOA', number_points=5, degree_fit = 4, plot_curve = True, base_bot_percentage = 32, base_top_percentage = 8, **kwargs):
    ...
    points_spectrum = np.linspace(int(len(x)/number_points), len(x)-int(len(x)/number_points), number_points)
    
    for point in points_spectrum:
        
        idx,val = find_nearest_val (x, x[int(point)])
        plt.plot (x[idx],y[idx], 'ko')

            # calculates the weight according to a trapezoid function 
        x_new, y_new = trapezoid_weight (x, y, val, base_bot_percentage=base_bot_percentage, base_top_percentage=base_top_percentage)
            # calculates the center of mass of the new curve
        cx,cy = give_curve_center_mass (x_new, y_new, degree_fit = degree_fit, plot_curve = plot_curve)
            # saves the values
...
def eff_wvl_exp( spectrum_arr, tof_points = [], degree_fit = 4, base_bot_percentage = 32, base_top_percentage = 8, plot_curve = True, **kwargs):
    
        # get the spectrum and separate it into x and y lists
    x = spectrum_arr[:,0]
    y = spectrum_arr[:,1]

        #determine the min and max values in the tof_points list
    spectrum_period = len(x)
    min_idx = int(np.floor(0.005*base_bot_percentage*spectrum_period))
    max_idx = int(np.floor(spectrum_period-0.005*base_bot_percentage*spectrum_period))
    ...
    for value in tof_points:
        
        idx,val = find_nearest_val (x, value)
        plt.plot (x[idx],y[idx], 'ko')
        if idx <= min_idx or idx>= max_idx:

            print ('The value '+ str(value) + ' is out of the minimum or maximum boundaries determined by the trapezoid base (min: ' + str(x[min_idx+1]) + ' & max: ' + str(x[max_idx-1]) +'. Please choose a different point.')
            break
...
def eff_wvl_exp_values(tof_values_exp, beamline_wvl_values, flight_path=1, **kwargs):
    ...
    for val in tof_values_exp:
        ...
        idx,closest_val = find_nearest_val (beamline_tof_values, val)
        #new_tof_exp_values.append(min(beamline_tof_values, key=lambda x:abs(x-val)))
        new_tof_exp_values. append(closest_val)
